#ifndef CANARD_REPEAT_ALTERNATE_CONDITIONAL_P_CENTER_H
#define CANARD_REPEAT_ALTERNATE_CONDITIONAL_P_CENTER_H

#include "Canard/graph/repeat_alternate_p_center.hpp"
...
namespace Canard {
  namespace detail {
    template <class WeightMap, class IndexMap,
      ...
      struct RepeatAlternatePCenter {
        RepeatAlternatePCenter(std::size_t times, WeightMap w, IndexMap i,
            Compare cmp, Combine cmb, DistInf inf, DistZero zero)
          : w_map(w), i_map(i), compare(cmp), combine(cmb), inf(inf), zero(zero)
        { }
        template <class Graph, class CenterMap,
          ...
          {
            return repeat_alternate_p_center(g, p, times, center_map, w_map,
                i_map, candidate_map, demand_map, compare, combine, inf, zero);
          }
        std::size_t times;
        WeightMap w_map;
        IndexMap i_map;
    ...
    template <class WeightMap, class IndexMap,
              class Compare, class Combine, class DistInf, class DistZero>
      inline RepeatAlternatePCenter<
        WeightMap, IndexMap, Compare, Combine, DistInf, DistZero
      >
      make_repeat_alternate_p_center_func
      (std::size_t times, WeightMap w, IndexMap i,
       Compare cmp, Combine cmb, DistInf inf, DistZero zero)
      {
        return RepeatAlternatePCenter<
            WeightMap, IndexMap, Compare, Combine, DistInf, DistZero
  ...
  template <class Graph, class CenterMap, class ExistedCenterMap,
            ...
            class Compare, class Combine, class DistInf, class DistZero>
    typename boost::property_traits<WeightMap>::value_type
    repeat_alternate_conditional_p_center
     ...
     Compare compare, Combine combine, DistInf inf, DistZero zero)
    {
      return conditional_p_center(g, p, center_map,
          detail::make_repeat_alternate_p_center_func(times, w_map, i_map,
  ...
  template <class Graph, class CenterMap, class ExistedCenterMap,
     ...
     CandidateMap candidate_map, DemandMap demand_map)
    {
      ...
      return repeat_alternate_conditional_p_center(g, p, times, center_map,
          existed_center_map, w_map, i_map, candidate_map, demand_map,
...
#endif // CANARD_REPEAT_ALTERNATE_CONDITIONAL_P_CENTER_H
