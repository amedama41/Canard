...
namespace Canard {
  template <class Graph, class CenterMap,
     ...
     WeightMap w_map, IndexMap i_map, ColorMap c_map)
    {
      ...
      boost::copy(vertices(g),
          Canard::make_property_map_iterator(g_map, vertices(g).first));
      ...
      const DistValue average
        = boost::accumulate(dist, DistValue()) / dist.size();
      boost::transform(dist, dist.begin(), boost::lambda::_1 - average);
      return boost::inner_product(dist, dist, DistValue()) / 2;
    }
     ...
     const boost::bgl_named_params<Param, Tag, Rest>& params)
    {
      ...
      return calc_uneven_distribution(
          ...
          boost::choose_param(
            ...
            make_iterator_property_map(predecessor.begin(), index_map)),
  ...
  template <class Graph, class CenterMap,
     ...
     const boost::bgl_named_params<Param, Tag, Rest>& params)
    {
      ...
      return calc_uneven_distribution(g, center_map,
          boost::make_iterator_property_map(generator.begin(), index_map),
          weight_map, index_map, color_map,
          params);
    }
