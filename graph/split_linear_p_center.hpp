...
namespace Canard {
  template <class Graph, class CenterMap, class WeightMap, class EdgeColorMap,
     ...
     ExistedCenterMap existed_center_map, CandidateMap candidate_map)
    {
      ...
      typedef detail::LinearPCenterGraphInfo<LinearGraph> LinearGInfo;

      LinearGraphs linear_graphs
        = split_to_linear_graphs(g, color_map, existed_center_map);

      // Guarantee that not 2 degree vertices are exisited centers.
      DummyCenterMap<Graph, ExistedCenterMap>
        dummy_center_map(g, existed_center_map);

      std::vector<LinearGInfo> infos;
      infos.reserve(linear_graphs.size());

      std::priority_queue<
        LinearGInfo*, std::vector<LinearGInfo*>, indirected_less<LinearGInfo>
      > que;

      for (typename LinearGraphs::iterator it = linear_graphs.begin();
          it != linear_graphs.end(); ++it) {
        infos.push_back(LinearGInfo
            (*it, dummy_center_map, candidate_map, w_map, i_map));
      ...
      for (std::size_t i = 0; i < p; ++i) {
        ...
        info.cover_dist
          = Canard::glpk_p_center(*(info.g), info.num_center, center_map,
              w_map, i_map, dummy_center_map, candidate_map);
