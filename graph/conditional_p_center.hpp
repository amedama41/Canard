...
namespace Canard {
  namespace detail {
    template <class DistanceMap, class Compare>
      ...
      public:
        ...
        dist_greater(DistanceMap d_map, Compare compare)
    ...
    template <class DistanceMap, class Compare>
      inline dist_greater<DistanceMap, Compare>
      make_dist_greater(DistanceMap d_map, Compare cmp)
      { return dist_greater<DistanceMap, Compare>(d_map, cmp); }
    ...
    template <class DemandMap, class DemandVec>
      ...
      {
        boost::fill(
            demands | Canard::adaptors::mapped(d_map)
                    | boost::adaptors::sliced(0, r),
            true);
        boost::fill(
            demands | Canard::adaptors::mapped(d_map)
                    | boost::adaptors::sliced(r, boost::distance(demands)),
            false);
      }
  } // detail
  ...
  template <class Graph, class CenterMap, class ExistedCenterMap,
     ...
     Compare compare, Combine combine, DistInf inf, DistZero zero)
    {
      ...
      boost::sort(demand_vertices, detail::make_dist_greater(d_map, compare));
