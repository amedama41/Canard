  ...
  template <class Graph,
    ...
    struct LinearGraphs {
      public:
        ...
        explicit LinearGraphs(const allocator_type& a)
          : container(a) { }

        explicit LinearGraphs(size_type n)
          : container(n) { }

        LinearGraphs(size_type n, const value_type& value,
            const allocator_type& a = allocator_type())
          : container(n, value, a) { }

        LinearGraphs(const LinearGraphs& lg)
          : container(lg.container) { }

#ifdef  __GXX_EXPERIMENTAL_CXX0X__
        LinearGraphs(LinearGraphs&& lg)
          : container(std::move(lg.container)) { }

        LinearGraphs(std::initializer_list<value_type> l,
            const allocator_type& a = allocator_type())
          : container(l, a) { }
#endif

        template <typename InputIterator>
          LinearGraphs(InputIterator first, InputIterator last,
              const allocator_type& a = allocator_type())
          : container(first, last, a) { }

        LinearGraphs& operator=(const LinearGraphs& lg)
        {
          container = lg.container;
          return *this;
        }

        bool operator==(const LinearGraphs& lg)
